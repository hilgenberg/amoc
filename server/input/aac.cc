...
static int buffer_fill_frame(struct aac_data *data)
{
	...
	while (1) {
		...
		datap = (unsigned char*) buffer_data(data);
...
static int aac_count_time (struct aac_data *data)
{
	...
	while (frames < 50) {
		...
		sample_buf = (int16_t*) NeAACDecDecode (data->decoder, &frame_info,
		                             (unsigned char*) buffer_data (data), buffer_length (data));
...
static void *aac_open_internal (struct io_stream *stream, const char *fname)
{
	...
	n = NeAACDecInit (data->decoder, (unsigned char*) buffer_data(data), buffer_length(data),
...
static void *aac_open (const char *file)
{
	...
	data = (aac_data*) aac_open_internal (NULL, file);
...
static int decode_one_frame (struct aac_data *data, void *buffer, int count)
{
	...
	aac_data = (unsigned char*) buffer_data (data);
	aac_data_size = buffer_length (data);

	/* aac data -> raw pcm */
	sample_buf = (char*)NeAACDecDecode (data->decoder, &frame_info,
